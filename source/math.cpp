...
scalar_t min(const scalar_t& a, const scalar_t& b)
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    return mcut_mpf_min(a, b);
#else
    return std::min(a, b);
#endif
}
...
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    return mcut_mpf_max(a, b);
#else
    return std::max(a, b);
#endif
}

scalar_t square_root(const scalar_t& number)
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    return mcut_mpf_sqrt(number);
#else
...
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    return mcut_mpf_abs(number);
#else
    return std::fabs(number);
#endif // #if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
}

sign_t sign(const scalar_t& number)
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    int s = mcut_mpf_sgn(number);
#else
...
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    const scalar_t& acx = pa[0] - pc[0];
    const scalar_t& bcx = pb[0] - pc[0];
    const scalar_t& acy = pa[1] - pc[1];
    const scalar_t& bcy = pb[1] - pc[1];
...
{
#if defined(MCUT_USE_MULTI_PRECISION_FLOAT_INTERFACE)
    const scalar_t& adx = pa[0] - pd[0];
    const scalar_t& bdx = pb[0] - pd[0];
    const scalar_t& cdx = pc[0] - pd[0];
    const scalar_t& ady = pa[1] - pd[1];
    const scalar_t& bdy = pb[1] - pd[1];
    const scalar_t& cdy = pc[1] - pd[1];
    const scalar_t& adz = pa[2] - pd[2];
    const scalar_t& bdz = pb[2] - pd[2];
    const scalar_t& cdz = pc[2] - pd[2];
    ...
    const int polygon_normal_largest_component)
{
    ...
    matrix_t<scalar_t> I(3, 3); // 3x3 identity
    I(0, 0) = 1.0;
    I(1, 1) = 1.0;
    I(2, 2) = 1.0;
