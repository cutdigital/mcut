        ...
        if (new_ivertices_count == 2) { // one edge
            ...
            for (std::vector<vd_t>::const_iterator iter = sorted_descriptors.cbegin() + 1; iter != sorted_descriptors.cend(); ++iter) {
                ...
                if (cutpath_edge_exists == false) {
                    ...
                    const std::vector<fd_t> src_vertex_faces = ps_get_ivtx_registry_entry_faces(ps, m0_ivtx_to_intersection_registry_entry[(std::size_t)src_vertex - ps_vtx_cnt] /*find_iter->second*/);
                    ...
                    const std::vector<fd_t> tgt_vertex_faces = ps_get_ivtx_registry_entry_faces(ps, m0_ivtx_to_intersection_registry_entry[(std::size_t)tgt_vertex - ps_vtx_cnt] /*find_iter->second*/);
