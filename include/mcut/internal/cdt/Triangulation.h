    ...
    const Edge originalEdge)
{
    ...
    tie(iT, iVleft, iVright) = intersectedTriangle(iA, aTris, a, b, distanceTolerance);
    ...
    while (std::find(t.vertices.begin(), t.vertices.end(), iB) == t.vertices.end()) {
        ...
        if (m_intersectingEdgesStrategy == IntersectingConstraintEdges::Resolve && fixedEdges.count(Edge(iVleft, iVright))) {
            ...
            const unordered_map<Edge, EdgeVec>::const_iterator originalsIt = pieceToOriginals.find(splitEdge);
            ...
            std::stack<TriInd> triStack = insertPointOnEdge(iNewVert, iT, iTopo);
    ...
    const TriInd iTleft = triangulatePseudopolygon(iA, iB, ptsLeft.begin(), ptsLeft.end());
    std::reverse(ptsRight.begin(), ptsRight.end());
    const TriInd iTright = triangulatePseudopolygon(iB, iA, ptsRight.begin(), ptsRight.end());
    ...
    const BoundaryOverlapCount overlaps)
{
    ...
    for (std::vector<Edge>::const_iterator it = flippedFixedEdges.begin();
        ...
        const unordered_map<Edge, EdgeVec>::const_iterator originalsIt = pieceToOriginals.find(*it);
    ...
    const VertInd iVert)
{
    ...
    std::stack<TriInd> triStack = trisAt[1] == noNeighbor
    ...
    const V2d<T>& pos) const
{
    ...
    while (!found) {
        ...
        for (Index i_(0); i_ < Index(3); ++i_) {
            ...
            if (edgeCheck == PtLineLocation::Right && t.neighbors[i] != noNeighbor && visited.insert(t.neighbors[i]).second) {
                found = false;
