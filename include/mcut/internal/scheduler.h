...
namespace mcut
{
    ...
    public:
        thread_safe_queue() : head(new node), tail(head.get()), done(false) {}
    ...
    class thread_pool
    {
        ...
        std::vector<std::thread> threads; // NOTE: must be declared after "done" and "work_queues"
        join_threads joiner;
        unsigned int round_robin_scheduling_counter;
    ...
    public:
        thread_pool() : done(false), joiner(threads), round_robin_scheduling_counter(0)
        {
    ...
    */
        ...
        std::future<typename std::result_of<FunctionType()>::type> submit(FunctionType f)
        {
            ...
            unsigned int worker_thread_id = (round_robin_scheduling_counter++) % get_num_threads();
