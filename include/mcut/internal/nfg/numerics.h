...
class interval_number
{
	...
	static inline __m128d minus_one()
	{
...
#	ifdef USE_AVX2_INSTRUCTIONS
	int sign() const
	{
		...
		__m128i r = _mm_castpd_si128(_mm_blendv_pd(
			_mm_castsi128_pd(_mm_set1_epi32(1)), _mm_castsi128_pd(_mm_set1_epi32(-1)), m));
...
#	endif
	...
	typedef union error_approx_type_t
	{
		...
		inline error_approx_type_t(double a)
			: d(a)
		{ }
		inline uint64_t is_negative() const
		{
			return u >> 63;
		}
	} casted_double;
...
public:
	...
	interval_number() { }
	interval_number(double a)
		: min_low(-a)
		, high(a)
	{ }
	interval_number(double minf, double sup)
		: min_low(minf)
		, high(sup)
	{ }
	interval_number(const interval_number& b)
...
public:
	...
	static void Two_One_Sum(
		const double a1, const double a0, const double b, double& x2, double& x1, double& x0)
	{
		double _i;
		Two_Sum(a0, b, _i, x0);
		Two_Sum(a1, _i, x2, x1);
	...
	static void Two_Product_2Presplit(
		double a, double _ah, double _al, double b, double _bh, double _bl, double& x, double& y)
	{
		x = a * b;
		y = (_al * _bl) - (((x - _ah * _bh) - (_al * _bh)) - (_ah * _bl));
	}
	...
	static void Two_One_Prod(const double a1,
							 const double a0,
							 const double b,
							 double& x3,
							 double& x2,
	...
	static int
	Gen_Sum_With_Alloc(const int elen, const double* e, const int flen, const double* f, double** h)
	{
	...
	static int Gen_Diff_With_Alloc(
		const int elen, const double* e, const int flen, const double* f, double** h)
	{
		*h = AllocDoubles(elen + flen);
	...
	static int Gen_Product_With_Alloc(
		const int alen, const double* a, const int blen, const double* b, double** h);
	...
	static int Gen_Scale_With_PreAlloc(
		const int elen, const double* e, const double& b, double** h, const int hlen);
...
public:
	...
	inline const uint32_t& back() const
	{
