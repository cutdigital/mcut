...
static uint32_t __inline clz_(uint32_t value)
{
...
namespace bvh {
    ...
    static unsigned int clz(unsigned int x) // stub
    {
#ifdef _MSC_VER
        return clz_(x);
#else
    ...
    static int get_leaf_level_from_real_leaf_count(const int t)
    {
    ...
    static int get_level_from_implicit_idx(const int bvhNodeImplicitIndex)
    {
        return ilog2(bvhNodeImplicitIndex + 1);
    }

    // compute previous power of two
    static unsigned int flp2(unsigned int x) // prev pow2
    {
        x = x | (x >> 1);
        x = x | (x >> 2);
        x = x | (x >> 4);
    ...
    static int get_ostensibly_implicit_bvh_size(const int t)
    {
        return 2 * t - 1 + __builtin_popcount(next_power_of_two(t) - t);
    }

    // compute left-most node on a given level
    static int get_level_leftmost_node(const int node_level)
    {
        return (1 << node_level) - 1;
    }

    // compute right-most leaf node in tree
    static int get_rightmost_real_leaf(const int bvhLeafLevelIndex, const int num_real_leaf_nodes_in_bvh)
    {
        return (get_level_leftmost_node(bvhLeafLevelIndex) + num_real_leaf_nodes_in_bvh) - 1;
    }

    // check if node is a "real node"
    static bool is_real_implicit_tree_node_id(const int bvhNodeImplicitIndex, const int num_real_leaf_nodes_in_bvh)
    {
    ...
    static int get_level_rightmost_real_node(
