...
*/
    ...
    public:
        ...
        public:
            ...
            I cend()
            {
                return cend<I>(identity<I>()); // https://stackoverflow.com/questions/3052579/explicit-specialization-in-non-namespace-scope
            }
        ...
        private:
            template <typename I>
            I cend(const identity<I>)
            {
                return I(); // unused
            }

            key_iterator_t<face_map_t> cend(identity<face_map_t> i_)
            {
                return mesh_ptr->faces_end();
            }

            key_iterator_t<edge_map_t> cend(identity<edge_map_t> i_)
            {
                return mesh_ptr->edges_end();
            }

            key_iterator_t<halfedge_map_t> cend(identity<halfedge_map_t> i_)
            {
                return mesh_ptr->halfedges_end();
            }

            key_iterator_t<vertex_map_t> cend(identity<vertex_map_t> i_)
            {
