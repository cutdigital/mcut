...
/*
This tutorial shows how to propagate per-triangulated0face normals (flat shading) from input meshes and onto the output 
triangulated connected components after cutting.
*/
...
int main()
{
    ...
    for (int i = 0; i < (int)connectedComponents.size(); ++i) {
        ...
        //  query the [triangulated] faces
        // -------------------

        numBytes = 0;
        err = mcGetConnectedComponentData(context, connComp, MC_CONNECTED_COMPONENT_DATA_FACE_TRIANGULATION, 0, NULL, &numBytes);
        my_assert(err == MC_NO_ERROR);
        std::vector<uint32_t> ccTriangulatedFaceIndices(numBytes / sizeof(uint32_t), 0);
        err = mcGetConnectedComponentData(context, connComp, MC_CONNECTED_COMPONENT_DATA_FACE_TRIANGULATION, numBytes, ccTriangulatedFaceIndices.data(), NULL);
        my_assert(err == MC_NO_ERROR);
        ...
        //  query the triangulated face map
        // -----------------------
        const uint32_t ccTriangulatedFaceCount = static_cast<uint32_t>(ccTriangulatedFaceIndices.size()/3);
        ...
        // Here we create and name the connected component based on its properties
        ...
        if (ccType == MC_CONNECTED_COMPONENT_TYPE_SEAM) {
            ...
            if (isFragment) {
                ...
                my_assert(err == MC_NO_ERROR);
                name += sType == MC_FRAGMENT_SEAL_TYPE_COMPLETE ? ".sealed" : ".unsealed";
            }
        }

        bool ccIsFromSrcMesh = (ccType == MC_CONNECTED_COMPONENT_TYPE_FRAGMENT);

        // connected-component is not a fragment && it is a seam
        if (!ccIsFromSrcMesh) {
            if (ccType == MC_CONNECTED_COMPONENT_TYPE_SEAM) {
                // get origin
        ...
        std::vector<int> ccTriangleVertexNormalIndices; // normal indices reference by each triangle

        // for each triangle in cc
        for (int t = 0; t < (int)ccTriangulatedFaceCount; ++t) {
            ...
            const uint32_t imFaceIdxRaw = ccFaceMap.at(t); // source- or cut-mesh face index (we don't know yet)
            // ** This is how we infer which mapped indices belong to the source-mesh or the cut-mesh.
            // NOTE: it is generally advisable to use the number of face of the internal source-mesh since MCUT might modify
            ...
            int faceSize = 3; /// number of vertices in face
            ...
            for (int v = 0; v < faceSize; ++v) {
                ...
                if (isSeamVertex) { // normal completely unknown and must be inferred/computed
                ...
                ccTriangleVertexNormalIndices.push_back(normalIndex);
            } // for (int v = 0; v < faceSize; ++v) {

            faceVertexOffsetBase += faceSize;
        } // for (int f = 0; f < ccTriangulatedFaceCount; ++f) {
        ...
        for (int k = 0; k < (int)ccTriangulatedFaceCount; ++k) {
            int faceSize = 3;

            file << "f ";
            for (int j = 0; j < faceSize; ++j) {
                const int idx = faceVertexOffsetBase + j;
                const int ccVertexIdx = ccTriangulatedFaceIndices.at(static_cast<size_t>(idx));
                file << (ccVertexIdx + 1) << "//" << ccTriangleVertexNormalIndices[idx] + 1 << " ";
            }
            file << std::endl;
