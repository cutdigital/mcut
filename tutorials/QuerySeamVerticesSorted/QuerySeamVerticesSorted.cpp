...
void readOFF(const char *fpath, double **pVertices, unsigned int **pFaceIndices,
             unsigned int **pFaceSizes, unsigned int *numVertices,
             unsigned int *numFaces);

void writeOFF(const char *fpath, const double *pVertices,
              const uint32_t *pFaceIndices, const uint32_t *pFaceSizes,
...
void read_OBJ_file(
  ...
  do { // each iteration will parse the full file
    ...
    int faceIndicesCounter = 0; // running offset into pFaceIndices (final value
                                // will be same as nFaceIndices)

    // number of characters read on a line
    size_t nread = 0;

    while ((nread = getline(&line, &lineBufLen, file)) !=
           -1) { // each iteration will parse a line in the file
      ...
      const bool lineIsComment = line[0] == '#';
      ...
      case ObjFileCmdType::VERTEX: {
        ...
        if (passIterator == 1) {
          ...
          nread = sscanf(line + 2, "%lf %lf %lf", &x, &y, &z);
          ...
          if (nread != 3) {
            fprintf(stderr, "error: have %d components for v%zu\n", nread,
                    vertexId);
            std::abort();
          }
        }
      ...
      case ObjFileCmdType::NORMAL: {
        ...
        if (passIterator == 1) {
          ...
          (*pVertices)[normalId * 3 + 0] = x;
          (*pVertices)[normalId * 3 + 1] = y;
          (*pVertices)[normalId * 3 + 2] = z;

          if (nread != 3) {
            fprintf(stderr, "error: have %d components for vn%zu\n", nread,
                    normalId);
            std::abort();
          }
        }
      ...
      case ObjFileCmdType::TEXCOORD: {
        int texCoordId =
            nTexCoords++; // incremental vertex-normal count in file

        if (passIterator == 1) {

          double x = 0.0;
          double y = 0.0;
          nread = sscanf(line + 2, "%lf %lf", &x, &y);

          (*pVertices)[texCoordId * 2 + 0] = x;
          (*pVertices)[texCoordId * 2 + 1] = y;

          if (nread != 2) {
            fprintf(stderr, "error: have %d components for vt%zu\n", nread,
                    texCoordId);
            std::abort();
          }
        }
      ...
      case ObjFileCmdType::FACE: {
        ...
        if (passIterator == 2) // third pass
        {
          ...
          while (pch != NULL) {
            ...
            while (pch_ !=
                   NULL) { // an iteration parses all data of a face vertex

              int val;
              sscanf(pch_, "%d", &val); // extract face vertex data str
              pch_ = strtok(NULL, "/");
    ...
    {
      if (nFaceIndices == 0) {
        fprintf(stderr, "error: invalid face index count %u\n", nFaceIndices);
        std::abort();
      }
    ...
    if (fsetpos(file, &startOfFile) !=
        0) /* reset current position to start of file */
    {
      if (ferror(file)) {
        perror("fsetpos()");
...
int main() {
  {
    ...
    read_OBJ_file(DATA_DIR "/brad/source-mesh.obj", &pVertices, &pNormals,
                  &pTexcCoords, &pFaceIndices, &pFaceSizes, &numVertices,
                  &numNormals, &numTexCoords, &numFaces, true);
  ...
  for (int i = 0; i < (int)connComps.size(); ++i) {
    ...
    const uint32_t numSeamVertexSequences =
        seamVertexSequenceArrayFromMCUT[runningOffset++]; // always the first
                                                          // element
    ...
    for (uint32_t numSeamVertexSequenceIter = 0;
      ...
      std::pair<std::vector<McUint32>, McBool> &currentSeamVertexSequenceData =
          seamVertexSequences.back();

      std::vector<McUint32> &currentSeamVertexSequenceIndices =
      ...
      const uint32_t currentSeamVertexSequenceIndicesArraySize =
          seamVertexSequenceArrayFromMCUT[runningOffset++];
    ...
    for (uint32_t j = 0; j < seamVertexSequences.size(); ++j) {
      ...
      flags_str.append(
          "-id" + std::to_string(j) +
          ((seamVertexSequences[j].second == MC_TRUE) ? "_isLOOP" : "_isOPEN"));
    }
...
void readOFF(const char *fpath, double **pVertices, unsigned int **pFaceIndices,
  ...
  *pFaceSizes = (unsigned int *)malloc(sizeof(unsigned int) * (*numFaces));
  ...
  (*pFaceIndices) =
      (unsigned int *)malloc(sizeof(unsigned int) * numFaceIndices);
