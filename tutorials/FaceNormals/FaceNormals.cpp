...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < (int)connComps.size(); ++i) {
        ...
        std::vector<Eigen::Vector3d> ccVertexNormals;
        ...
        std::vector<int> ccFaceVertexNormalIndices;
        ...
        for (int f = 0; f < ccFaceCount; ++f) {
            ...
            bool reverseWindingOrder = (isFragment && isWaterTight && pathLocation == MC_PATCH_LOCATION_OUTSIDE);
            ...
            for (int v = 0; v < faceSize; ++v) {
                ...
                if (isSeamVertex) { // normal unknown and must be computed
                ...
                } else { // normal is known must be inferred from input mesh
                ...
                std::vector<Eigen::Vector3d>::const_iterator fiter = std::find_if(
                  ccVertexNormals.cbegin(), ccVertexNormals.cend(), 
                  [&](const Eigen::Vector3d& e) {return compare(e.x(), normal.x()) && compare(e.y(), normal.y()) && compare(e.z(), normal.z()); });

                if (fiter != ccVertexNormals.cend())
                {
                    normalIndex = std::distance(ccVertexNormals.cbegin(), fiter);
                }

                if (normalIndex == -1) { // normal not yet stored for CC vertex in face
                    normalIndex = ccVertexNormals.size();
                    ccVertexNormals.push_back(normal);
                }
        ...
        for (int i = 0; i < ccVertexNormals.size(); ++i) {
